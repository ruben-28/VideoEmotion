...
def _get_video_urls(video_name: str) -> dict:
    viz_h264 = (
        ...
        / video_name
        / f"{video_name}_annotated_h264.mp4"
    )
    viz_raw = (
        settings.server.PROJECT_ROOT
        / settings.project.paths.visualizations
        / video_name
        / f"{video_name}_annotated_raw.mp4"
    )
    realtime_h264 = (
        settings.server.PROJECT_ROOT
    ...
    if viz_h264.exists():
        video_url = f"/static/output/visualizations/{video_name}/{video_name}_annotated_h264.mp4"
    elif viz_raw.exists():
        video_url = (
            f"/static/output/visualizations/{video_name}/{video_name}_annotated_raw.mp4"
        )
    elif realtime_h264.exists():
        video_url = f"/static/output/realtime/{video_name}/session_h264.mp4"
    else:
        video_url = f"/static/videos/{video_name}/{video_name}.mp4"

    return {
        "video_url": video_url,
        "thumbnail_url": f"/static/output/thumbnails/{video_name}.jpg",
    }
...
async def list_videos(
    ...
    status: Optional[str] = Query(
        None, description="Filter by status: processed, partial, unprocessed"
    ),
    ...
    try:
        ...
        videos = video_manager.list_videos(
            mode=video_mode, status=video_status, sort_by=sort_by, sort_order=sort_order
        )
        ...
        for v in paginated_videos:
            video_responses.append(
                VideoResponse(
                    ...
                    file_size_mb=round(v.file_size_bytes / (1024 * 1024), 2)
                    if v.file_size_bytes
                    else None,
                    **_get_video_urls(v.name),
                    stats=v.stats,
                )
            )
