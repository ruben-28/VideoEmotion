...
def get_video_manager() -> VideoManager:
    ...
    return VideoManager(
        ...
        scanner=VideoScanner(
            ...
            realtime_dir=settings.server.PROJECT_ROOT / settings.project.paths.realtime_output
        ),
        store=MetadataStore(
...
def get_trash_manager() -> TrashManager:
    return TrashManager(
        project_root=settings.server.PROJECT_ROOT,
        trash_dir=settings.server.PROJECT_ROOT / "trash", # Assuming trash dir is fixed or needs config
        metadata_store=MetadataStore(settings.server.PROJECT_ROOT / settings.project.paths.metadata)
    )
