...
#     # ✅ NEW: résumer uniquement une session (ex: nom vidéo)
#     parser.add_argument(
#         "--only-session",
#         default=None,
...
def main() -> None:
    ...
    parser.add_argument(
        "--only-session",
        default=None,
        help="Ne générer que le résumé pour ce groupe (ex: 'test_pipeline' ou 'gemini2').",
    )
    ...
    # ✅ Filtre robuste sur une session
    if args.only_session:
        wanted = args.only_session.strip().lower()

        def match(label: str) -> bool:
            lbl = label.strip().lower().replace("\\", "/")
            parts = [p for p in lbl.split("/") if p]
            return (lbl.startswith(wanted) or wanted in parts)

        groups = {k: v for k, v in groups.items() if match(k)}
        if not groups:
            print(f"[ERREUR] Aucune session ne correspond à --only-session={args.only_session}")
            return
