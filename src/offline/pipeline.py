...
#     # Aggregate JSON -> INSIDE per-video folder (no more duplicates at root)
#     emotion_scan_root = (emotion_video_root / frames_dir) if (emotion_video_root / frames_dir).exists() else emotion_video_root
...
#     # bbox json expected: detected_video_root/frames_fpsX/bboxes.json
...
#         # give root; visualize_results.py creates per-video folder too, but it will reuse the same one
#         "--out", str(visualizations_root),
#         "--use-smoothed",
#         "--show-confidence",
#         "--show-backend",
#         "--bbox-fps", str(fps),
#         # copy bboxes/results next to video output folder.
#         # Results is already inside per-video folder; this won't create root duplicates anymore.
#         "--copy-jsons",
#     ]
