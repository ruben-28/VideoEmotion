...
class StatsUpdater:
    ...
    async def recalculate_all_stats_async(self) -> bool:
        ...
        offline_task = loop.run_in_executor(
            self._executor, self._recalculate_offline_stats
        )
        realtime_task = loop.run_in_executor(
            self._executor, self._recalculate_realtime_stats
        )
