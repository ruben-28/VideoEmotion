...
from .models import PipelineJob, PipelineConfig, PipelineProgress, JobStatus, VideoStatus
...
class PipelineExecutor:
    ...
    def execute_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            # Execute
            logger.info(f"=" * 80)
            logger.info(f"PIPELINE EXECUTION DEBUG INFO:")
    ...
    def _update_video_status(self, video_name: str, status: VideoStatus) -> None:
        ...
        try:
            ...
            for vid, vdata in metadata.get("videos", {}).items():
                if vdata.get("name") == video_name or vdata.get("name") == Path(video_name).stem:
                    video_id = vid
                    break
            ...
            if status == VideoStatus.PROCESSED:
                metadata["videos"][video_id]["processed_at"] = datetime.now().isoformat()
