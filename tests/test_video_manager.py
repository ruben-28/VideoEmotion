...
def test_scan_videos_sync(video_manager, mock_scanner, mock_store, mock_stats, tmp_path):
...
@pytest.mark.asyncio
async def test_scan_videos_async(video_manager, mock_scanner, mock_store, mock_stats, tmp_path):
...
def test_list_videos_filtering(video_manager, mock_store):
    ...
    now = datetime.now()
    v1 = VideoMetadata(
        id="v1", name="v1", mode=VideoMode.OFFLINE, status=VideoStatus.PROCESSED, 
        created_at=now, file_paths={}, file_size_bytes=0, stats={}
    )
    v2 = VideoMetadata(
        id="v2", name="v2", mode=VideoMode.REALTIME, status=VideoStatus.UNPROCESSED, 
        created_at=now, file_paths={}, file_size_bytes=0, stats={}
    )
